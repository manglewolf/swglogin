...
from includes.db_connect import get_connection, cursor
...
app.secret_key = os.environ.get('FLASK_SECRET_KEY', "u8V7fX9s2aQy4mLr6ZpTj3NqBvC1hKs0_eG5wRzY6oP8dS9xU2cF4nM7bH0tV1")
...
if not logger.handlers:
    handler = RotatingFileHandler(logs_dir / 'auth.log', maxBytes=5 * 1024 * 1024, backupCount=3)
...
def index():
    ...
    return render_template('index.html', user=user, statuses=statuses, online_players=online_players)
...
def new_user_post():
    ...
    try:
        ...
        with cursor() as cur:
            cur.execute("SELECT 1 FROM user_account WHERE username = %s", (useraccountname,))
            if cur.fetchone():
...
def auth_php():
    ...
    logger.info('Auth attempt for username=%s, stationID=%s, ip=%s', username, station_id, ip)
    ...
    if not user:
        ...
        return jsonify({'message': 'Account does not exist or password was incorrect'}), 401

    if user.get('accesslevel') == 'banned':
        logger.info('Auth rejected: banned account %s', username)
        return jsonify({'message': 'Your account has been banned. For further information regarding the ban of your account or to submit a Ban Appeal, contact a member of CSR Staff.'}), 403
...
def post_login():
    ...
    logger.info('Login attempt: username=%s, stationID=%s, ip=%s', username, stationID, ip)
...
def post_change_password():
    ...
    if session.get('username', '').lower() != username.lower() and session.get('accesslevel') != 'superadmin':
